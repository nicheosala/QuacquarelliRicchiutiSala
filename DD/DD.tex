	...
	\section{Architectural design}
		...
		\subsection{Selected architectural styles and patterns}
			\subsubsection{Model-View-Controller (MVC)}
				...
				\begin{itemize}
					\item \textit{Model}: the central component of the pattern. It represents the application's data structure, independent of the user interface. It directly manages the data, logic and rules of the application.
					\item \textit{View}: any form of data representation. 
					\item \textit{Controller}: accepts input and converts it to commands for the model or view
				\end{itemize}
				
				One of the biggest advantages of MVC is the possibility to have multiple views of the same information. Thinking about SafeStreets, both users and authorities have an own GUI and different level of details about data.
				it is based on separation
				This division of concerns makes the application easier to test and maintain: requests first arrive at the controller (or some of them) which binds the model with the
				corresponding view. 
				In fact, an other advantage is the possibility of parallelize application development among workers, such that groups can focus on a fixed part, i.e. model, view and controller.\\
				
				Focusing more on SafeStreets, the MVC division is also observable in Figure \ref{fig:component_diagram}. The backend of each application, both client and server side, is part of \textit{controller}; SafeStreets user interfaces represent the \textit{view} and they are just a mean to let users and authorities to communicate with the \textit{model}, information stored in SafeStreets databases.\\\\
				...
				In fact, with \textit{fat} or \textit{thick} clients there is no need for continuous server communications (e.g. violation reporting for users involve locally an OCR algorithm to verify a correct reading of license plate, without contacting server and generate more workload).
			...
			\subsubsection{RESTful web services}
				...
				an architectural style for communication based on strict use of HTTP request types (Figure\ref{fig:RESTful web services}) .\\
				...
				A resource is an item of interest, exposed to the clients. In SafeStreets context, resources examples can be violation reports, accidents or suggestions for authorities. \\
