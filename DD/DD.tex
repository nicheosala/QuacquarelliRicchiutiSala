...
\begin{document}
		...
		\subsection{Revision history}
			\begin{table}[h]
				...
				\begin{tabular}{c c c}
					...
					\hline
					1.0 &  \nth{9} December, 2019  & \\ \hline
					2.0 &  \nth{30} January, 2020  & Name of two interface methods corrected\\
					\hline
				\end{tabular}
	...
	\section{Architectural design}
		...
		Basically everyone can download the SafeStreets application on his smartphone and can be a \textit{user}. On the other hand the Authority Edition is distributed by SafeStreets only to certificated \textit{authorities} that wants to collaborate with it. 
		\begin{figure}[H]
		...
		This layer contains the SafeStreets App back-end, the SafeStreets AE back-end and the SafeStreets Server. The two back-ends communicate only with the server. This is important because only the server can exchange messages with the database system of the data layer, ensuring an higher level of security not allowing individuals to access directly to sensitive data.\\
		\\
		The \textit{Data layer} simply contains the Data Management System, that collects and execute queries from relational databases. All the queries results are sent only to SafeStreets server.
		...
		\end{figure}
   		...
   		Other kind of DBMS are also allowed. The key aspect is that the database structure must be relational to satisfy the functionalities required by the system that needs a fixed data structure to return specific kind of information or it needs to aggregate related information to make the server able to generate suggestions.
		...
		\begin{itemize}
			\item \textbf{Anonymous data}\\
				...
				\begin{itemize}
					\item{getAnonymousViolationReports}\\\\
					\begin{tabular}{l | l}
